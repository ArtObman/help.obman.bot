...
General support queries and possible bugs in OBMAN.BOT are **not** in scope and will be closed without comment; use the
[support server] instead.

[support server]: https://discord.gg/5GJCKpuMuE

## Submission Guide

Thanks for contributing to the OBMAN.BOT documentation! Please take a moment to review this document in order to make the
...
````markdown
```yag
{{/* your OBMAN.BOT custom command code here */}}
```
````
