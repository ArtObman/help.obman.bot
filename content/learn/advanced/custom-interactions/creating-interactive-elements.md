...
Modals are a pop-up form that OBMAN.BOT can send in response to an interaction.
It allows users to privately input text which is sent directly to OBMAN.BOT for use in your custom command.
...
{{ $button3 := cbutton "label" "Duck Three" "emoji" (sdict "name" "🦆") "url" "https://obman.bot" "style" "link" }}
...
The solution is to tell OBMAN.BOT precisely how the rows look like and then pass each row to the `"button"` or `"menu"` key.
...
As you continue to attach more components, OBMAN.BOT will automatically overflow into new rows.
...
As you probably already discovered when building embeds, you can use `sdict` in favor of `cembed` to create some sort of skeleton of your embed for later adjustments, with OBMAN.BOT's template engine automagically handling the conversion for you.
