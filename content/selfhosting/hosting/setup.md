...
On this page, we will explain how to let Discord know about your bot and to configure OBMAN.BOT accordingly.

## Discord

Every Discord user requires an identity, the same goes for any bot on Discord.
Follow the below steps to setup OBMAN.BOT on the Discord side of your deployment.
...
Without those, your instance of OBMAN.BOT will not work.
...
Create them with the following URIs, where `OBMAN.BOT_HOST` is the domain or IP you wish to use to access the control panel.
Of course, if you only intend to have the bot be local to your machine, `localhost` also works.

- `OBMAN.BOT_HOST/confirm_login`
- `OBMAN.BOT_HOST/manage`

If you're unsure yet, leave out this step and come back later.

## Obtain OBMAN.BOT Sources

Clone the Git repository from <https://github.combotlabs-gg/OBMAN.BOT> and change into the clone.
