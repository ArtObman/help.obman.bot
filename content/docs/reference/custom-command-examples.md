...
If you plan to copy paste codes for your server, please see the [community-ran repository](https://OBMAN.BOT-cc.github.io) of custom commands, which is frequently updated with newly contributed commands.
...
Please take notice, some of examples presented here are not up to date with current capabilities of OBMAN.BOT.
Please visit our [support server](/discord) for newer solutions.

### Controlled randomizer example

OBMAN.BOT has a built-in random response system for custom commands, but sometimes you may want to control the chances for certain responses to occur.
...
{{ $slice := cslice "OBMAN.BOT " "is " "cool!" }}
...
A dictionary does not currently have a lot of practical use, because OBMAN.BOT has a data type more suited for most use cases - `sdict`.
...
With OBMAN.BOT's database system, you can now add cooldowns to you custom commands.
...
With OBMAN.BOT's database system, I made a command to have users count from 0 and keep counting to the next number.
...
This will also work for emotes that are from the servers OBMAN.BOT is not in, as it gets the emote file directly from Discord's database.
