...
All available data that can be used in OBMAN.BOT's templating "engine" which is slightly modified version of Go's stdlib text/template package.
...
| .IsPremium     | Returns boolean true/false whether guild is premium of OBMAN.BOT or not.                                                                                                                                                                                                                                               |
...
Use of interactions within OBMAN.BOT is an advanced topic; the documentation should be used only as reference.
...
**{{/* range over a slice */}}
**{{ range $index, $element := cslice "OBMAN.BOT" "IS COOL!" }}
{{ $index }} : {{ $element }} {{ end }}
{{/* range on a map */}}
...
OBMAN.BOT's templating "engine" has currently two user-defined, custom data types - _templates.Slice_ and _templates.SDict_.
There are other custom data types used like _discordgo.Timestamp_, but these are outside of the main code of OBMAN.BOT, so not explained here further.
...
- To demonstrate .StringSlice `{{(cslice currentTime.Month 42 "OBMAN.BOT").StringSlice}}` will return a slice `[February OBMAN.BOT]`.
...
| .HasKey "key"    | Returns _bool_ true/false regarding whether the key is set or not e.g. `{{(sdict "OBMAN.BOT" "is cool").HasKey "OBMAN.BOT"}}` would return `true`. |
...
Go by Example lists [some examples of using *time.Time*](https://gobyexample.com/time) in Go code, which can be referenced for similar use in OBMAN.BOT's templates.
