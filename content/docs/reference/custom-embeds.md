...
A guide to creating custom embeds in various contexts across OBMAN.BOT.
...
One method of sending an embed with OBMAN.BOT is using the command `customembed` (or for short, `ce`).

### Create embeds by hand

OBMAN.BOT accepts embeds in JSON or YAML following the rules of Discord's [Embed Object] structure.
...
To make your code readable, especially for large embeds, **indents** may be used, as OBMAN.BOT's templating system allows this sort of formatting.
