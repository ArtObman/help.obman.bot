...
weight = 230
description = "Configure command overrides to restrict access to OBMAN.BOT commands and optional autodelete intervals."
+++

Fine-grained control over all of OBMAN.BOT's inbuild commands.

## Overview

Command overrides allow you to restrict access to OBMAN.BOT's inbuilt commands and optionally configure intervals after which the command trigger and response will be autodeleted.
...
The prefix (**1**) is a short sequence of characters that trigger OBMAN.BOT commands.
...
In addition to the command prefix, you can trigger OBMAN.BOT commands by pinging the bot at the start of your message.
This is helpful if you forget your prefix, as sending `@obman.bot prefix` will recall it.
...
OBMAN.BOT will then either require all members to have any of these roles in order to run commands, or completely ignore members with any of the ignored roles, server admins and owners included.

{{< callout context="note" title="Note: Priority of Ignored vs. Required Roles" icon="outline/info-circle" >}}

OBMAN.BOT was raised well and honors a "no" when told "no".
...
This setting makes OBMAN.BOT automatically delete the triggering message and/or its response after the configured duration has passed.
...
Toggling this setting on will make OBMAN.BOT always respond to slash commands with an ephemeral message.
...
Toggling this option simply tells OBMAN.BOT whether the commands you selected are enabled or disabled.
