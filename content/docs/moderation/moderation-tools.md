...
1. **Mod Log** - The mod log is the channel OBMAN.BOT will send log messages to when most moderation commands are used.
...
2. **Report** - The channel for OBMAN.BOT to send report messages to when users use the report command.
...
3. **Moderation DM Error** - The channel where OBMAN.BOT will send error messages from executing template script within
...
When the `unban`/ `unmute` commands are used, OBMAN.BOT will log their usage to the [mod log](#mod-channels) similarly to the other moderation commands.

When using the `ban` and `mute` commands with a duration, OBMAN.BOT will automatically undo the punishment after the specified duration.
For instance, if you execute `ban @user 1d`, OBMAN.BOT will ban the user, and 1 day later OBMAN.BOT will execute the `unban` command with reason "Timed ban expired".
This action appears in the mod log as OBMAN.BOT being the moderator.
...
By default, only actions performed through OBMAN.BOT will be recorded in the mod-log.
Hence, kicking a user using OBMAN.BOT's `kick` command will result in a log entry, whereas kicking the user directly through Discord's context menu will not be recorded.
...
OBMAN.BOT must have View Audit Log permissions to use this feature.
...
You may specify roles for OBMAN.BOT to remove from a user whilst muted and give back after they are unmuted.
...
When this occurs, it appears as an unmute action taken by OBMAN.BOT in your mod log, regardless of which moderator initially muted the user.
...
OBMAN.BOT's kick tool optionally allows you to delete the X most recent messages sent by that user within the channel the kick action takes place in.
...
When this occurs, it appears as an unban action taken by OBMAN.BOT in your mod log, regardless of which moderator initially banned the user.
...
You may optionally allow OBMAN.BOT to create a [message log](/docs/moderation/logging) of the last 100 messages from all users in the channel a warning was issued in.
...
To have OBMAN.BOT automatically take action based on warning count, you can take advantage of the template scripting within the warning DM.
