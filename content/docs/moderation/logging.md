...
OBMAN.BOT IS NOT A LOGGING BOT.

{{< /callout >}}

![A message log from the OBMAN.BOT support server](message_log_example.png)

Message logs capture X amount of messages from a channel, including any deleted messages cached by OBMAN.BOT (the last 1 hour of deleted messages, or last 12 hours with [premium](/docs/welcome/premium), are recorded by OBMAN.BOT).

The log is accessible on OBMAN.BOT's website for later viewing.
...
Message logs on the official OBMAN.BOT instance will be automatically deleted after 30 days of their creation.
...
OBMAN.BOT cannot send message logs to a Discord channel; they are only accessible online.
...
When viewing a log, OBMAN.BOT colors the usernames of each message author based on their roles at the time of log _viewing_, similarly to how they would appear on your server.
...
If you are self-hosting your own instance of OBMAN.BOT, you can set `enable_message_log_purge=true` to automatically purge message logs older than 30 days.
This option is enabled on the official instance of OBMAN.BOT hosted by Botlabs but is disabled by default on a fresh self-host.
