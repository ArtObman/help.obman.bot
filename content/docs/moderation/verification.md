...
If a user has disabled DMs from server members by default, upon joining your server OBMAN.BOT will fail to send a verification DM to the user.
It is recommended that you inform your new members about this possibility and how to fix it if they do not receive a DM from OBMAN.BOT.

{{< /callout >}}

## Kick After Unverified

You may optionally configure OBMAN.BOT to kick users who haven't verified after X minutes.
...
Alt Detection is currently disabled on the official instance of OBMAN.BOT hosted by Botlabs.
These features will not function on the official bot.

{{< /callout >}}

By default, upon successful verification OBMAN.BOT will check the user's IP (as captured by the verification page) against other users who have verified using OBMAN.BOT.

### Alt Banning

When a user is banned for any reason (including bans made outside the bot), OBMAN.BOT will automatically ban any alt accounts detected for that user.

If a new user verifies who shares an IP with a previously banned user, they will be banned as well.

### Alt Reporting

If a new user who verifies is detected as an alt but none of their associated accounts have been banned, OBMAN.BOT will send a log to the verification log channel if specified, identifying the new user as an alt and listing other users who verified at the same IP address.
